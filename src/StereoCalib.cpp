...
void parseCommandline(const int &argc, char **argv, std::string &input,
    ...
    desc.add_options()("help,h", "this help message")("input,i",
            ...
            boost::program_options::value<std::string>(),
            "Output file with calibration values for cameras")("length,l",
...
bool getCorners(const cv::Mat &image, const cv::Size &chessboardSize,
...
int main(int argc, char **argv) {
    ...
    double leftReprojectionError = cv::calibrateCamera(modelPoints,leftPoints,imageSize,lCM,lDC,rvecs,tvecs);
    rvecs.clear();
    tvecs.clear();
    double rightReprojectionError = cv::calibrateCamera(modelPoints,rightPoints,imageSize,rCM,rDC,rvecs,tvecs);

    std::cerr<<"left rms"<<leftReprojectionError<<std::endl;
    std::cerr<<"right rms"<<rightReprojectionError<<std::endl;


    double reprojectionError = cv::stereoCalibrate(modelPoints, leftPoints,
            rightPoints, lCM, lDC, rCM, rDC, imageSize, R, T, E, F,
            cv::TermCriteria(CV_TERMCRIT_ITER + CV_TERMCRIT_EPS, 50, 1e-6), CV_CALIB_FIX_INTRINSIC);
    std::cerr << reprojectionError << std::endl;

    saveCalibParameters(output,lCM,rCM,lDC,rDC,R,T,E,F,chessboardSize,imageSize,sideLength);
}
