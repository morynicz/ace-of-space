...
int main(int argc, char** argv)
{
  ...
  cv::Mat projMatL, projMatR, rectL, rectR, disparityToDepthMap, lCM, rCM, lDC,
    rDC, R, T, E, F, lR, rR, lP, rP, Q;
  ...
  cv::Rect rightRoi(cv::Point(imageSize.width, 0), imageSize);
  ...
  cv::Ptr<cv::stereo::StereoBinarySGBM> sbm =
    cv::stereo::StereoBinarySGBM::create(0, 3, 16);

  cv::stereoRectify(lCM, lDC, rCM, rDC, imageSize, R, T, lR, rR, lP, rP, Q, 0);
  cv::initUndistortRectifyMap(
    lCM, lDC, lR, lP, imageSize, CV_16SC2, lMap1, lMap2);
  cv::initUndistortRectifyMap(
    rCM, rDC, rR, rP, imageSize, CV_16SC2, rMap1, rMap2);

  capL.set(cv::CAP_PROP_FRAME_WIDTH, imageSize.width);
  capL.set(cv::CAP_PROP_FRAME_HEIGHT, imageSize.height);
  capR.set(cv::CAP_PROP_FRAME_WIDTH, imageSize.width);
  capR.set(cv::CAP_PROP_FRAME_HEIGHT, imageSize.height);
  ...
  do
  {
...

    {
      ...
      cv::triangulatePoints(lP, rP, keyPointsLS, keyPointsRS, homogenousPoints);

      if (!keyPointsL.empty() && !keyPointsR.empty() && !goodMatches.empty())
      {
        cv::drawMatches(uL, keyPointsL, uR, keyPointsR, goodMatches, matched);
