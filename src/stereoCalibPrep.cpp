...
{
  cv::Size leftImageSize(leftCapture.get(cv::CAP_PROP_FRAME_WIDTH),
                         leftCapture.get(cv::CAP_PROP_FRAME_HEIGHT));

  cv::Size rightImageSize(rightCapture.get(cv::CAP_PROP_FRAME_WIDTH),
                          rightCapture.get(cv::CAP_PROP_FRAME_HEIGHT));

  cv::Size result = leftImageSize;

  if (leftImageSize.height < rightImageSize.height)
  {
    rightCapture.set(cv::CAP_PROP_FRAME_HEIGHT, leftImageSize.height);
  }
  else if (leftImageSize.height > rightImageSize.height)
  {
    leftCapture.set(cv::CAP_PROP_FRAME_HEIGHT, rightImageSize.height);
    result.height = rightImageSize.height;
  }

  if (leftImageSize.width < rightImageSize.width)
  {
    rightCapture.set(cv::CAP_PROP_FRAME_WIDTH, leftImageSize.width);
  }
  else if (leftImageSize.width > rightImageSize.width)
  {
    leftCapture.set(cv::CAP_PROP_FRAME_WIDTH, rightImageSize.width);
...
int main(int argc, char** argv)
{
  ...
  parseCommandline(
    argc, argv, target, leftDevice, rightDevice, chessboardSize, sideLenght);

  leftCapture.open(leftDevice);
  if (!leftCapture.isOpened())
  {
    cv::Exception ex(
      0, "Didn't open left device", __func__, __FILE__, __LINE__);
    throw ex;
  }
  ...
  {
    cv::Exception ex(
      0, "Didn't open right device", __func__, __FILE__, __LINE__);
    throw ex;
  }
